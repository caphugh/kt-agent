...
.claude/commands/ingest.md    — Processes inbox → knowledge base (sole writer)
.claude/commands/interview.md — Conducts knowledge interviews → inbox
.claude/commands/cleanse.md   — Audits knowledge quality → correction sidecars
.claude/skills/parse/         — Converts raw files to markdown
.claude/skills/categorize/    — Assigns category, metadata, splits multi-topic
.claude/skills/persona/       — Updates persona.md
...
| Command | Purpose |
|---------|---------|
| `/interview` | Conduct a focused knowledge interview on one topic. Outputs transcript to inbox/. |
| `/ingest` | Process all inbox files into the knowledge base. Calls parse → categorize → persona skills. |
| `/cleanse` | Audit knowledge base quality. Outputs correction sidecars to inbox/. |
...
- **Single writer:** Only `/ingest` writes to `knowledge/`
...
`/interview` → answer questions → `/ingest` → `/cleanse`

**Adding knowledge via file upload:**
Drop file in `inbox/` → `/ingest` → `/cleanse`

**Querying knowledge:**
Invoke `kt-agent` → ask questions → agent retrieves from knowledge base

**Maintaining quality:**
`/cleanse full` → review sidecars → `/ingest` to apply corrections
